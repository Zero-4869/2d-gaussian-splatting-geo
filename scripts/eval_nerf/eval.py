...
def load_quad_as_tri_cpu(file_path):
    # Use a library like trimesh to get the raw quad data on CPU
    import trimesh
    q_mesh = trimesh.load(file_path, process=False)
    if isinstance(q_mesh, trimesh.Scene):
    # This merges all geometries in the scene into one single Trimesh object
        q_mesh = q_mesh.to_geometry()
    else:
        q_mesh = q_mesh
    
    verts = np.array(q_mesh.vertices)
    faces = np.array(q_mesh.faces) # This will be (N, 4) if it's a quad mesh
    
    if faces.shape[1] == 4:
        # Split: [v0, v1, v2, v3] -> [v0, v1, v2] and [v0, v2, v3]
        tri1 = faces[:, [0, 1, 2]]
        tri2 = faces[:, [0, 2, 3]]
...
if __name__ == '__main__':
    ...
    if args.mode == 'mesh':
        ...
        data_mesh = load_quad_as_tri_cpu(args.data)
    ...
    gt_mesh_path = os.path.join(args.dataset_dir, f'{args.scan}/{args.scan}.obj')
    # gt_mesh = o3d.io.read_triangle_mesh(gt_mesh_path)
    gt_mesh = load_quad_as_tri_cpu(gt_mesh_path)
